...
struct FreeRoamView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            // Check queue status to ensure it's being refilled
        ...
        .onChange(of: state.isCurrentPuzzleCompleted) { completed in
            if !completed {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.05) {
                    showCelebrationCard = true
                    // Check queue status when celebration card appears
                    state.checkQueueStatus()
                }
    ...
    private var mainContent: some View {
        VStack(spacing: ResponsiveSizing.spacing(baseSpacing: 6)) {
            HStack {
                Button(action: {
                    presentationMode.wrappedValue.dismiss()
                }) {
                    Image(systemName: "chevron.left")
                        .responsiveFont(size: 22, weight: .bold, design: .rounded)
                        .foregroundColor(Color("C_Charcoal"))
                        .frame(width: ResponsiveSizing.tileSize(for: 4) * 0.84, height: ResponsiveSizing.tileSize(for: 4) * 0.84)
                ...
                Button(action: {
                    ...
                    Image(systemName: "questionmark.circle")
                        .responsiveFont(size: 20, weight: .medium)
                        .foregroundColor(Color("C_Charcoal"))
                        .frame(width: ResponsiveSizing.tileSize(for: 4) * 0.84, height: ResponsiveSizing.tileSize(for: 4) * 0.84)
                        .contentShape(Rectangle())
                }
            ...
            Text("Free Roam")
                .responsiveFont(size: 34, weight: .bold, design: .rounded)
                .foregroundColor(Color("C_Charcoal"))
                .responsivePadding(.top, 4)
                ...
                ForEach(lengthOptions, id: \.self) { option in
                    Button(action: {
                        ...
                        Text(lengthLabels[option] ?? "")
                            ...
                            .shadow(color: state.currentWordLength == option ? Color("C_WarmTeal").opacity(0.10) : .clear, radius: ResponsiveSizing.spacing(baseSpacing: 4), x: 0, y: ResponsiveSizing.spacing(baseSpacing: 2))
                    }
                    .buttonStyle(PlainButtonStyle())
                }
            }
            .responsivePadding(.horizontal, 0)
            .responsivePadding(.bottom, 16)
            ...
            if !chain.isEmpty {
                        ...
                        AnyView(
                            Button(action: {
                                ...
                                Image(systemName: "arrow.right")
                                    .responsiveFont(size: 16, weight: .bold, design: .rounded)
                                    .foregroundColor(.white)
                            }
                            .buttonStyle(PlainButtonStyle())
                        )
                ...
                .frame(height: ResponsiveSizing.tileSize(for: 4) * 4.2) // Responsive height based on tile size
                .responsivePadding(.top, 32)
                if !chain.isEmpty {
                    LetterKeyboard(
